...
function analyzeSalesData(data, options) {
    ...
    const purchaseData = data.purchase_record || data.purchase_records || data.purchases || [];
    
    if (!data
        || ((!Array.isArray(data.sellers)) && (!Array.isArray(data.products)) && (!Array.isArray(purchaseData)))
        || ((data.sellers.length === 0) && (data.products.length === 0) && (purchaseData.length === 0))
    ) {
    ...
    if (Array.isArray(purchaseData)) {
        purchaseData.forEach(record => {
            ...
            items.forEach(item => {
                ...
                const cost = (product.purchase_price || product.cost || 0) * (item.quantity || 1);
